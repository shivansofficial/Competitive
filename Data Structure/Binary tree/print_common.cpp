...
bool print_ancestor(node * root,int d)
{
  if (root==NULL)
    return false;
  if(root->data==d)
  ...
  if(print_ancestor(root->left,d)||print_ancestor(root->right,d))
    {
      cout<<root->data<<" ";
      return true;
    }
...
bool find_common(node * root,int n1,int n2)
{
  node * lca=lca_util(root,n1,n2);
  if(!lca)
    return false;
  print_ancestor(root,lca->data);
  return true;
}
int main()
{
  ...
  root->right->left->right = new_node(10);
  root->left->right->right = new_node(11);
  int n1,n2;
  cin>>n1>>n2;
  if(!find_common(root,n1,n2))
